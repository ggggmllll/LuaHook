# LuaFFI: 动态C函数绑定库

一个基于 libffi 的 Lua 扩展库，实现 Lua 与 C 的双向高效调用。支持结构体传递、可变参数函数、ABI 选择等特性，无需编写胶水代码即可直接调用任意 C 函数或将 Lua 函数暴露给 C 使用。

## ✨ 主要特性

- **双向调用**：
  - 将任意 C 函数指针包装为 Lua 可调用对象（`wrapNative`）
  - 将 Lua 函数包装为 C 函数指针（`wrapLua`），通过 libffi closure 实现
- **结构体支持**：
  - 注册 C 结构体布局，Lua 中用数组表示结构体（按字段顺序）
  - 自动计算字段偏移量，正确处理对齐
- **可变参数函数**：
  - C 函数签名支持 `...` 标记，可变参数类型由最后一个固定参数决定并自动提升（`float`→`double`，小于 `int` 的整数→`int`/`unsigned int`）
  - Lua 闭包包装时不支持可变参数
- **ABI 选择**：可动态设置调用约定（通过 `setAbi`）
- **类型安全**：签名解析 + 运行时类型检查，错误信息清晰
- **内存管理**：
  - C 函数包装返回 full userdata，Lua GC 自动回收
  - Lua 函数包装返回 lightuserdata，需手动释放（`unwrapLua`）
- **线程安全**：全局映射表使用互斥锁保护

## 📦 API 参考

所有函数通过模块表导出。

### `luahook.setAbi(abi)`
设置全局 FFI ABI 编号。参数为整数，取值范围为 `[FFI_FIRST_ABI, FFI_DEFAULT_ABI]`（由 libffi 定义）。通常使用 `0`（默认 ABI）。

### `luahook.registerStruct(name, signature)`
注册一个 C 结构体类型。
- `name`：字符串，结构体名称（后续签名中可用）
- `signature`：字符串，字段类型列表，例如 `"i|i|"` 表示两个 `int` 字段。
  签名格式：字段类型用单字符表示，之间用 `|` 分隔，末尾需有 `|`。

### `luahook.unregisterStruct(name)`
注销已注册的结构体。

### `luahook.wrapNative(ptr, signature) -> userdata`
将 C 函数指针包装为 Lua 可调用的对象。
- `ptr`：lightuserdata，C 函数地址
- `signature`：字符串，函数签名，格式 `"返回类型|参数1|参数2|...|"`。若为可变参数，在最后加 `...` 标记（例如 `"i|p|...|"`）。
- 返回值：full userdata，带有 `__call` 元方法，可直接在 Lua 中调用。

### `luahook.wrapLua(func_name, signature) -> lightuserdata`
将 Lua 函数包装为 C 函数指针（通过 libffi closure）。
- `func_name`：字符串，全局 Lua 函数名
- `signature`：字符串，签名格式同 `wrapNative`，但**不支持可变参数**（即不能包含 `...`）
- 返回值：lightuserdata，即生成的 C 函数可执行地址，可传递给需要 C 回调的 API。

### `luahook.unwrapLua(code)`
释放由 `wrapLua` 创建的闭包资源。
- `code`：lightuserdata，之前返回的可执行地址。

## 🔢 类型签名映射

### 基本类型单字符

| 字符 | C 类型              | Lua 类型   | 说明                     |
|------|---------------------|------------|--------------------------|
| `v`  | `void`              | `none`     | 仅用于返回值             |
| `c`  | `signed char`       | `integer`  |                          |
| `C`  | `unsigned char`     | `integer`  |                          |
| `s`  | `signed short`      | `integer`  |                          |
| `S`  | `unsigned short`    | `integer`  |                          |
| `i`  | `signed int`        | `integer`  |                          |
| `I`  | `unsigned int`      | `integer`  |                          |
| `l`  | `signed long`       | `integer`  |                          |
| `L`  | `unsigned long`     | `integer`  |                          |
| `f`  | `float`             | `number`   |                          |
| `d`  | `double`            | `number`   |                          |
| `p`  | `void*` / 指针      | `userdata` | lightuserdata 或 full userdata |
| `o`  | `long double`       | `number`   |                          |
| `F`  | `_Complex float`    | `number`   | 暂未特殊处理，按两个 number 处理 |
| `D`  | `_Complex double`   | `number`   |                          |
| `O`  | `_Complex long double` | `number` |                          |
| `...`| 可变参数标记        | -          | 仅用于 C 函数签名        |

### 结构体类型
签名中用注册时的名称代替字符，例如已注册结构体 `"Point"`，则签名中可用 `Point` 作为一个参数类型。

结构体在 Lua 中表示为**数组**，元素顺序与结构体字段声明顺序一致。嵌套结构体递归展开。

## 📝 使用示例

```lua
local ffi = require("luahook")

-- 设置 ABI（通常默认 0）
ffi.setAbi(0)

-- 注册结构体 Point { int x, int y }
ffi.registerStruct("Point", "i|i|")

-- 假设有一个 C 函数：int add(int a, int b);
-- ptr_add 是通过其他方式获得的 lightuserdata
local add = ffi.wrapNative(ptr_add, "i|i|")
local result = add(3, 5)   -- 返回 8

-- 假设 C 函数：void print_point(Point* p);
-- ptr_print_point 是函数指针
local print_point = ffi.wrapNative(ptr_print_point, "v|p|")
local point = {10, 20}     -- Lua 数组表示 Point
print_point(point)          -- C 函数接收指针，自动转换

-- 可变参数 C 函数：int sum(int count, ...);
local sum = ffi.wrapNative(ptr_sum, "i|i|...|")
-- 可变参数部分重复最后一个固定参数类型（int），并提升
print(sum(3, 1, 2, 3))      -- 输出 6

-- 将 Lua 函数包装为 C 回调
function lua_add(a, b)
    return a + b
end
local c_callback = ffi.wrapLua("lua_add", "i|i|i|")
-- 可将 c_callback 传递给需要 C 回调的 API
-- 不再使用时释放
ffi.unwrapLua(c_callback)
```

## 🔧 编译与依赖

### 依赖
- Lua 5.1 / 5.2 / 5.3 / 5.4
- libffi
- pthread（用于线程安全）

### 编译示例
手动编译：
```bash
cmake -B build && cd build && make
```

## ⚠️ 注意事项

1. **可变参数限制**：`wrapLua` 不支持可变参数签名，因为 libffi closure 无法处理。如需将 Lua 函数用作可变参数回调，需手动包装。
2. **结构体注册顺序**：使用结构体作为参数前必须先注册。
3. **指针生命周期**：`wrapNative` 传入的 C 函数指针必须在整个使用期间有效；Lua 端不负责管理 C 函数内存。
4. **线程安全**：全局映射表使用互斥锁，但 Lua 状态本身不是线程安全的，请勿在多线程中共享同一 Lua 状态调用本库。
5. **错误处理**：签名解析失败或类型不匹配会抛出 Lua 错误（`lua_error`）。

## 📄 许可证

MIT License

---

欢迎贡献 Issue 和 PR！